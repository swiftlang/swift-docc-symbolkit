...
extension UnifiedSymbolGraph {
    ...
    /// Iterates through the symbols in this graph and inserts new "overload group" symbols for
    /// any overloaded symbols found.
    ///
    /// An "overload" here is defined as any symbol whose path components and kind conflicts with
    /// another symbol. This method will collect all such conflicting symbols together and create a
    /// new "overload group" symbol, cloned from an arbitrarily-selected "first overload" and given
    /// a new unique identifier.
    internal func createOverloadGroups(restrictByLanguage languages: [String]? = nil) {
        struct OverloadKey: Hashable {
            let path: [String]
            let kind: SymbolGraph.Symbol.KindIdentifier
        }

        var symbolsByPath = [Selector: [OverloadKey: [UnifiedSymbolGraph.Symbol]]]()

        for symbol in self.symbols.values {
            for selector in symbol.mainGraphSelectors {
                // If we've been given a list of languages to restrict the collection with,
                // double-check that this selector is actually for one of those languages
                if let languages = languages, !languages.contains(selector.interfaceLanguage) {
                    continue
                }
                if let path = symbol.pathComponents[selector], let kind = symbol.kind[selector], kind.identifier.isOverloadableKind {
                    symbolsByPath[selector, default: .init()][.init(path: path, kind: kind.identifier), default: []].append(symbol)
                }
            }
        }

        for (selector, symbols) in symbolsByPath {
            var newRelationships = [SymbolGraph.Relationship]()

            for overloads in symbols.values where overloads.count > 1 {
                let firstOverload = overloads.first!
                ...
                for relationship in self.relationshipsByLanguage[selector] ?? [] where relationship.source == firstOverload.uniqueIdentifier {
                    var newRelationship = relationship
                    newRelationship.source = overloadGroupIdentifier
                    newRelationships.append(newRelationship)
                }
                for relationship in self.relationshipsByLanguage[selector] ?? [] where relationship.target == firstOverload.uniqueIdentifier {
                    var newRelationship = relationship
                ...
                for overloadSymbol in overloads {
                    newRelationships.append(.init(
                        source: overloadSymbol.uniqueIdentifier,
            ...
            if newRelationships.count > 0 {
                self.relationshipsByLanguage[selector, default: []].append(contentsOf: newRelationships)
            }
        }
    }
}
